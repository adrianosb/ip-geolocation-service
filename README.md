...
- **Ports and adapters** -- four interfaces in `application/port/`: `GeolocationUseCase` (input), `GeolocationPort` (external API), `IpValidationPort` (IP validation), and `GeolocationCachePort` (cache). The application layer has no imports from infrastructure.
